...
def gemini_response(chat_session, user_prompt):
    try:
        ...
        print(f"✅ Gemini response success: {response.text}")
        return response.text
    except Exception as e:
        print(f"❌ Gemini response fail: {e}")
...
async def make_call(request: CallRequest):
    try:
        ...
        print(f"📞 Phone call succeeded! call_sid: {call.sid} to: {request.to_number}")
        return {"success": True, "call_sid": call.sid}

    except Exception as e:
        print(f"❌ phone call fail: {e}")
        return {"success": False, "error": str(e)}


# --- Make a phone call test page ---
@app.get("/make-call")
async def make_call_page():
    html = """
    <html>
    <body>
        <h2>HelloMeds - Test</h2>
        ...
        <button onclick="makeCall()" style="padding:10px 20px;font-size:16px">📞 call</button>
        <p id="result"></p>
        <script>
    ...
    try:
        while True:
            ...
            if message["type"] == "setup":
                ...
                print(f"✅ Setup complete - call_sid: {call_sid}, to: {to_number}")
                ...
                print(f"👤 Patient info: {patient_name}, {medication}, {dose}")
...
Start the conversation by greeting {patient_name} and reminding them to take {dose} {medication}."""
                ...
                try:
                    ...
                    print(f"❌ Gemini session creation failure: {e}")

            elif message["type"] == "prompt":
                user_prompt = message.get("voicePrompt", "")
                print(f"🎤 User says: '{user_prompt}'")

                if not call_sid or call_sid not in sessions:
                    print(f"❌ No session! call_sid: {call_sid}")
                    continue
...
if __name__ == "__main__":
    print(f"🚀 server starts - Port: {PORT}")
